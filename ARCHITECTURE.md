...
| `simulated` flag (never ambiguous which mode a provider is in) | `packages/core/src/service.test.ts` + `packages/gateway/src/server.test.ts` — defaults to `true` for every provider with no credentials, persisted on the `Sandbox` through `get()`/`list()`, not just the create response |
| Real provider behavior (Modal, Vercel) — full CLI lifecycle | `examples/cli-e2e-live.sh` spawns the actual built `osr` binary against live infra: `providers`/`plan`/`create` (plain, named, `pin:`)/`exec` (with and without `--`)/`pause`/`resume`/`snapshot`/`--from-snapshot` restore/`rm`. Not run in CI (needs live credentials); this is what caught a real bug — Vercel's `pause()` (`stop()`) only auto-snapshots a `persistent` sandbox, so resuming an unnamed (non-persistent) sandbox failed with the provider's own `snapshot_not_found` until `pause()` was fixed to force persistence first. See the "Try it live" section of `docs/GUIDE.md`. |
